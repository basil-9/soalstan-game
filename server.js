...
// تحديد مجلد الملفات (index.html و questions.json يجب أن تكون داخل مجلد اسمه public)
...
let allQuestions = [];
try {
    const rawData = fs.readFileSync(path.join(__dirname, 'public', 'questions.json'), 'utf8');
    allQuestions = JSON.parse(rawData);
} catch (e) {
    console.error("⚠️ تأكد من وجود ملف questions.json داخل مجلد public");
}
...
io.on('connection', (socket) => {
    ...
    function calculateResults(room) {
        ...
        setTimeout(() => { startNextRound(room); }, 6000); // الانتقال للجولة التالية بعد 6 ثواني
    }
...
server.listen(PORT, () => {
    ...
    console.log(`👑 الرابط السري للإحصائيات: http://localhost:${PORT}/admin-stats?pass=king123`);
});
